...
; along with the FE3 register value that enables them.
;*******************************************************************************
...
;*******************************************************************************
; REG9C03: disable bits for the user's free-run configuration.
.export __memcfg_reg9c03
__memcfg_reg9c03: .byte 0
...
;*******************************************************************************
; APPLY
; Recomputes the FE3 disable bits for the current block mask.
; Must be called after every change to memcfg::blocks.
.export __memcfg_apply
.proc __memcfg_apply
	lda __memcfg_blocks
	ora #MEMCFG_BLK3	; native RESTORE trampoline must remain mapped
	eor #MEMCFG_ALL
	sta __memcfg_reg9c03
	rts
.endproc
...
@expanded:
	...
	and #MEMCFG_BLK3
	beq :+
	ldx #$7f		; keep BASIC below the native transition handlers
...
@activate:
	...
	cmp #MC_APPLY
	beq @apply
	cmp #3			; BLK3 contains the native RESTORE handler
	beq @key		; it cannot be disabled on FE3
